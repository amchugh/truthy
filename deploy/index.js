...
app.post('/api/feedback', cors(), function (req, res) {
    if (!req.query.domain || !req.query.bias || !req.query.reliability) {
    ...
    statement.get(req.query.domain, function (err, row) {
        ...
        if (row) {
            ...
            update_rating(
                req.query.domain, 
                row["bias"] * (1-feedback_weight) + req.query.bias * feedback_weight, 
                row["reliability"] * (1-feedback_weight) + req.query.reliability * feedback_weight
                );
        } else {
            console.debug("Could not find row for domain in /api/feedback", req.query.domain);
            on_domain_lookup_fail(req.query.domain)
            update_rating(req.query.domain, req.query.bias, req.query.reliability)
        }
        res.status(202).end()
    })
})
